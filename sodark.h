                                                ...
                                                uint64_t seed /*64 bits*/) noexcept
{
    ...
    A=I[1], B=I[2], C=I[3]
    r: 1...rounds
        A(r) = EBOX[A(r-1) xor B(r-1) xor K[(3 * r - 3) % 7 + 1] xor S[(3 * r - 3) % 8]]
        C(r) = EBOX[C(r-1) xor B(r-1) xor K[(3 * r - 2) % 7 + 1] xor S[(3 * r - 2) % 8]]
        B(r) = EBOX[B(r-1) xor A(r) xor C(r) xor K[(3 * r - 1) % 7 + 1] xor S[(3 * r - 1) % 8]]
    O={0,A,B,C}
    ******************************************************************************************/
                                                ...
                                                uint64_t seed /*64 bits*/) noexcept
{
    ...
    A=I[1], B=I[2], C=I[3]
    r: rounds...1
        B(r) = DBOX[B(r-1)] xor A(r-1) xor C(r-1) xor K[(3 * r - 1) % 7 + 1] xor S[(3 * r - 1) % 8]
        C(r) = DBOX[C(r-1)] xor B(r) xor K[(3 * r - 2) % 7 + 1] xor S[(3 * r - 2) % 8]
        A(r) = DBOX[A(r-1)] xor B(r) xor K[(3 * r - 3) % 7 + 1] xor S[(3 * r - 3) % 8]
    O={0,A,B,C}
    ******************************************************************************************/
